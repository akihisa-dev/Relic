...
- 実アプリの起動、操作、実画面確認、スクリーンショット、起動スモークは、ユーザーがその作業で明示的に指示した場合だけ実施する。UI変更や自動テストで判断できない状態があることだけを、実施許可とみなさない
- ユーザーがタグpushまたはリリースを明示した場合は、公開前検証として `pnpm verify:local:release` に含まれる、その作業で生成した配布版の自動起動スモークまで実施してよい。既存ウインドウ、配布済みアプリ、GUI操作の許可へは広げない
- 実アプリ確認の指示がない場合は、Electronを起動するコマンドを実行せず、対象テスト、型チェック、ビルド、文書整合、差分確認で検証する。実画面未確認を理由に完了を妨げず、必要なら未実施であることを報告する
- ユーザーが実アプリ確認を指示した場合は、その作業中に起動した開発版だけを対象とする。既存のRelicウインドウ、配布版、ビルド済みアプリは認識・起動・操作せず、通常の実テストのためにパッケージや配布ビルドを生成しない
