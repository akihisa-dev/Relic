...
| `nodes[].x` / `nodes[].y` | Node位置 |
| `nodes[].width` / `nodes[].height` | Nodeの固定サイズ |
| `nodes[].layer` | 図形ごとの表示レイヤー。領域図形は `0`、領域図形以外は `1` から `8` までとする。数値が大きいほど前面に表示し、見た目上も高く浮いて見える |
| `lines` | Node同士をつなぐLine一覧 |
| `lines[].id` | Diagramファイル内で一意のLine ID |
| `lines[].from` / `lines[].to` | 接続するNode ID |
| `lines[].label` | Line中央に表示する文字 |
...
- `0` は領域図形専用のレイヤーとし、領域図形以外の `nodes[].layer` は `1` から `8` までとして扱う
- 既存ファイルなどで `nodes[].layer` が未指定の場合、領域図形は `0`、領域図形以外は `1` として扱う
- 既存ファイルなどで領域図形に `0` 未満、領域図形以外に `1` 未満または `8` 超過の `nodes[].layer` が保存されている場合は、読み込み時に現在のルールへ合わせて扱う
...
- 領域図形以外の図形を選択している間だけ、選択図形の現在レイヤーを表示する
- レイヤー変更操作を行った直後は、選択図形とレイヤー表示を短く強調して操作結果が分かるようにする
- 領域図形以外の図形を背面へ送る場合もレイヤーは `1` 未満にはならず、前面へ送る場合もレイヤーは `8` を超えない。境界に達している場合、該当するレイヤー変更ボタンは無効にする
- 図形の見た目は、レイヤーが1段上がるごとに影、縁取り、下側の奥行き表現が明確に強くなり、位置をずらさずに浮いて見える
- 図形を選択している間、他の図形は選択図形より上のレイヤー、下のレイヤー、同じレイヤーの違いが分かる見た目にする
