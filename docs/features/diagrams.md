...
| `free-drawing` | Markdownファイル参照を持たない自由テキストNodeを配置・接続する |
...
FreeDrawingは、自由テキストNodeを配置し、Node同士をLineでつなぐ自由な図解である。
...
pan、zoom、選択状態、ドラッグ中の一時位置は画面だけの状態とし、Markdownファイルへ保存しない。
Node内テキスト、Node位置、Line追加、Line削除、Line方向変更、Node削除、Label編集はMarkdown本文へ書き戻す。
