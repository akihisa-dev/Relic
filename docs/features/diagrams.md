...
- 自由図用の図形一覧は、フローチャート寄りの開始/終了、処理、判断、入出力、ラベル、領域を表示する
...
- Lineは同じ行き先のもの、または空ではないLineラベルが完全一致するものだけ接続点や経路の合流を許可し、それ以外は同じNode辺でも接続点と経路を分ける
...
- Lineをダブルクリック、または選択中の空Lineラベルに出る追加操作からLineラベルを編集できる
- Line作成直後はLineラベル入力へ入る
...
Node位置、Line追加、Line削除、Line方向変更、Node削除、Lineラベル編集はMarkdown本文へ書き戻す。
...
- 自由図の表示層は固定とし、領域図形は `0`、領域以外の図形本体は `1`、図形に付くラベルは `2` として扱う
- 自由図では図形のレイヤーを前面または背面へ変更する操作を表示しない
- 判断図形や入出力図形などの非矩形図形では、選択中やドラッグ中も図形外側の四角い領域枠を表示しない
- Node移動、Nodeリサイズ、Line作成、Line削除、Line方向変更、Node削除、Lineラベル編集はRelationshipと同じ操作で扱う
- 空白部分をドラッグすると表示位置を動かせる
- ホイール操作でカーソル位置を基準に拡大縮小できる

pan、zoom、選択状態、ドラッグ中の一時位置は画面だけの状態とし、Markdownファイルへ保存しない。
Nodeの図形種類、Nodeラベル、Node位置、固定Nodeレイヤー、Line追加、Line削除、Line方向変更、Node削除、Lineラベル編集はMarkdown本文へ書き戻す。
...
Relationshipのような自由なNode配置、Line、Lineラベル、Node位置は持たない。
