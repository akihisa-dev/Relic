...
Diagramサイドバーから作成できるDiagramは、Relationshipと構造ツリーの2種類とする。
...
保存互換性のため、Diagram種別の `type` は `why-tree` を使う。
...
- `labelPreset` は `generic`、`analysis`、`thinking` のいずれかとする
- `generic` は `ルート` / `ノード` / `メモ` / `関連項目` / `アクション` を表示する既定ラベルセットとする
- `analysis` は `問題・現象` / `原因` / `根拠` / `解決策` / `実行項目` を表示する
- `thinking` は `テーマ` / `分解` / `情報` / `案` / `やること` を表示する
...
- キャンバス左側にラベルセット変更UIを浮かせて表示する
- ラベルセット変更UIで選んだ `labelPreset` はMarkdown本文へ書き戻す
- ルートの削除はできない
