...
- 待機時間そのものは所有するhookまたは純粋な状態遷移のテストで模擬時間を使って確認し、App統合テストでは同じ遅延を実時間で待ち直さない。統合側は表示や操作との接続など、その層でしか検出できない結果だけを残す。
