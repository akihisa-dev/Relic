...
| OS別package | Vitest外のOS別workflowで成果物の構造と起動を確認 |

末尾2役はVitestではなく、別プロセスで実行する確認責務を表す。Electron実行スモークは隔離した開発版を通常CIで起動し、OS別packageはPre-release VerificationとRelease workflowで実際の成果物を起動する。
