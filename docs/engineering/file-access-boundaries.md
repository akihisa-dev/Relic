...
ワークスペース変更通知では、絶対パスを送らず `workspaceId`、単調なrevision、`paths` または `full` の種別だけを送る。`paths` の場合も、Mainで正規化とワークスペース相対境界を確認したMarkdownパスだけを含める。Rendererへ渡る直前にも不正な値を検証し、危険または不明なパスはpathを捨てて全体再確認へ倒す。
...
| Markdownインポート | `app/src/main/files/markdownFileCreation.ts` | sourceは読み取りのみ、destinationは `resolveNewWorkspacePath(...)` と `verifyNewWorkspacePath(...)` |
...
- `../outside.md`、絶対パス、ドライブ文字付き絶対パス、NUL文字を入力検証で拒否する。
