...
| 暦面 | Calendar surface | `ChronicleCalendarSurface`, `range`, `chronicle.visibleCalendarSurfaces` | `app/src/shared/chronicleCalendar.ts`, `app/src/renderer/chronicleCanvasModel.ts`, `app/src/renderer/chronicleCanvasRenderer.ts` | [frontmatter](../features/frontmatter.md), [data-model](../engineering/data-model.md), [design](../design/DESIGN.md) |
...
### カードビュー
`card` 固定プロパティを持つMarkdownファイルを表示名、任意の代表画像、本文の `flavortext` で一覧する主要ビュー。`card` の値が空でも一覧へ含める。UI名は「カード」。メインエリアの `cards` チャートタブとして開く。
...
### 暦面
基準暦キャンバス上で、その他の暦が適用される開始年から終了年までの領域を表す横長の面。その暦で記述されたファイル項目と、その暦自身の年目盛りを面内に表示する。UI名は「暦面」。
...
### 機能トグル
設定パネルタブで切り替える機能のオン / オフ。現行実装では、カード、テーブル、グラフ、スフィア、クロニクルの表示・操作入口を制御する。旧 `frontmatter` と `tools` 値は読込互換のため保持するが表示制御には使わない。フロントマター設定はテーブルビューへ統合し、独立した機能トグルを設けない。ファイル加工、右パネルのアウトラインとリンクは常に利用可能であり、機能トグルの対象にしない。
