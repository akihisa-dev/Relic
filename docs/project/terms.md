...
| 年表チャート | Chronicle chart | `chronicle`, `ChartSource`, `WorkspaceChart`, `defaultCharts` | `app/src/shared/ipc.ts`, `app/src/main/files/charts.ts`, `app/src/renderer/components/ChartPanel.tsx` | [frontmatter](../features/frontmatter.md), [data-model](../engineering/data-model.md), [design](../design/DESIGN.md) |
...
### チャートタブ
年表をメインエリア内に開くタブ。`chronicle` のフロントマター値から表示行を生成する。
...
### 年表チャート
`chronicle` 固定プロパティと暦設定から生成するチャート表示。年または年月の期間をメイン暦の月単位の横軸上に表示し、バーの移動・伸縮でMarkdownファイルの元の `chronicle` エントリへ書き戻す。
...
### 暦設定
年表チャートで使うメイン暦とサブ暦を管理するパネルタブ。暦名とサブ暦のメイン暦開始年をワークスペース設定として保存する。Markdown本文には設定値を書き込まない。
