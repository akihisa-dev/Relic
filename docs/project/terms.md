...
| 固定プロパティ | Fixed property | `FixedFrontmatterFieldName`, `reservedFrontmatterFieldNames`, `aliases`, `category`, `tags`, `chronicle` | `app/src/shared/frontmatterFields.ts`, `app/src/renderer/frontmatterSettingsModel.ts` | [frontmatter](../features/frontmatter.md), [data-model](../engineering/data-model.md) |
...
| ビュー | View | `AppRailView`, `AppRailViewId` | `app/src/renderer/appShellModel.ts`, `app/src/renderer/components/railNavigationViews.tsx` | [navigation](../features/navigation.md), [design](../design/DESIGN.md) |
...
| チャートタブ | Chart tab | `ChartTab`, `kind: "chart"`, `openChartInPane`, `chart-sphere`, `chart-chronicle` | `app/src/renderer/store/editorStore.ts`, `app/src/renderer/store/editorStoreModel.ts`, `app/src/renderer/hooks/useAppTabRenderers.tsx` | [design](../design/DESIGN.md) |
| グラフビュー | Graph view | `GraphView`, `WorkspaceGraph`, `getWorkspaceGraphChannel`, `chartId: "graph"` | `app/src/renderer/components/GraphView.tsx`, `app/src/main/files/workspaceGraph.ts`, `app/src/shared/ipc/search.ts` | [navigation](../features/navigation.md), [data-model](../engineering/data-model.md), [design](../design/DESIGN.md) |
| スフィアビュー | Sphere view | `SphereView`, `SphereRuntime`, `chartId: "sphere"` | `app/src/renderer/components/SphereView.tsx`, `app/src/renderer/sphere/sphereRuntime.ts`, `app/src/renderer/sphere/sphereModel.ts` | [navigation](../features/navigation.md), [data-model](../engineering/data-model.md), [design](../design/DESIGN.md) |
| クロニクルビュー | Chronicle view | `chronicle`, `ChartSource`, `WorkspaceChart`, `chartId: "chronicle"` | `app/src/shared/ipc.ts`, `app/src/main/files/charts.ts`, `app/src/renderer/components/ChartPanel.tsx` | [frontmatter](../features/frontmatter.md), [data-model](../engineering/data-model.md), [design](../design/DESIGN.md) |
| 年表 | Timeline | `ChronicleCanvas`, `chronicle-canvas`, `chronicle.timelineAria` | `app/src/renderer/components/ChronicleCanvas.tsx`, `app/src/renderer/chronicleCanvasRenderer.ts` | [frontmatter](../features/frontmatter.md), [design](../design/DESIGN.md) |
...
| ファイル加工ツール | File tools | `tools`, `ToolsPanel`, `tools:*`, `generateTitleList`, `generateToc`, `generateTagIndex`, `mergeFiles` | `app/src/shared/ipc.ts`, `app/src/main/ipc/toolActions.ts`, `app/src/renderer/components/ToolsPanel.tsx` | [tools](../features/tools.md), [design](../design/DESIGN.md) |
...
| 機能トグル | Feature toggles | `FeatureToggles`, `featureToggles`, `tools`, `frontmatter`, `graph`, `sphere`, `chronicle`, `rightPanelOutline`, `rightPanelLinks` | `app/src/shared/ipc.ts`, `app/src/main/settings/appSettings.ts`, `app/src/renderer/components/SettingsPanel.tsx` | [design](../design/DESIGN.md), [development](../development.md) |
...
### ビュー
ユーザーが情報を見る目的ごとに切り替える、メインエリアの主要な表示単位。Relicの主要ビューは、Markdownを表示・編集する「ファイルビュー」、情報同士の関係を見る「グラフビュー」、同じ関係を3次元空間で見る「スフィアビュー」、情報を時間軸で見る「クロニクルビュー」の4つとする。ビューは仕様・設計・会話で使う概念名であり、実装上はファイルタブまたはチャートタブとしてメインエリアへ表示する。画面上では限られた表示幅で素早く識別できるよう、対応するUI名を「ファイル」「グラフ」「スフィア」「クロニクル」とする。

### ファイルビュー
Markdownファイルの内容を表示・編集する主要ビュー。UI名は「ファイル」。ファイルサイドバーで対象を選び、メインエリアのファイルタブへ表示する。ライブプレビューとソースモードは、ファイルビュー内のエディタ表示モードとして扱う。

### グラフビュー
ワークスペース内のMarkdown、タグ、添付画像、未解決リンクの関係をノードと線で表示する主要ビュー。UI名は「グラフ」。メインエリアのチャートタブとして開く。

### スフィアビュー
グラフビューと同じ関係データを、回転・拡大縮小できる3次元空間へ試験的に表示する独立した主要ビュー。UI名は「スフィア」。メインエリアの `sphere` チャートタブとして開き、2次元のグラフビューとは別の表示状態とライフサイクルを持つ。

### クロニクルビュー
`chronicle` 固定プロパティを持つMarkdownファイルを時間軸上の年表として表示する主要ビュー。UI名は「クロニクル」。メインエリアのチャートタブとして開き、年表はクロニクルビュー内の可視化表現として扱う。
...
### チャートタブ
グラフビュー、スフィアビュー、またはクロニクルビューをメインエリア内に開くタブ。
...
### ファイル加工ツール
既存Markdownファイルを読み取り専用で扱い、タイトル一覧、目次、タグ別索引、フォルダ内マージの結果を新規ファイルとして出力するパネルタブ。
...
### 機能トグル
設定パネルタブで切り替える機能のオン / オフ。現行実装では、ファイル加工ツール、フロントマター設定、グラフ、スフィア、クロニクル、右パネルのアウトライン、右パネルのリンクの表示・操作入口を制御する。
