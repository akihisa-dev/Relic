...
| 固定プロパティ | Fixed property | `FixedFrontmatterFieldName`, `reservedFrontmatterFieldNames`, `aliases`, `tags`, `chronicle` | `app/src/shared/frontmatterFields.ts`, `app/src/renderer/frontmatterSettingsModel.ts` | [frontmatter](../features/frontmatter.md), [data-model](../engineering/data-model.md) |
...
| グラフビュー | Graph view | `GraphView`, `WorkspaceGraph`, `getWorkspaceGraphChannel`, `chartId: "graph"` | `app/src/renderer/components/GraphView.tsx`, `app/src/main/files/workspaceGraph.ts`, `app/src/shared/ipcWorkspace.ts` | [navigation](../features/navigation.md), [data-model](../engineering/data-model.md), [design](../design/DESIGN.md) |
...
### 固定プロパティ
Relicが最初から特別扱いするフロントマターキー。`aliases`、`tags`、`chronicle` を指す。
...
### 暦設定
クロニクルビューで使うメイン暦とサブ暦を管理するパネルタブ。暦名とサブ暦のメイン暦開始年をワークスペース設定として保存する。Markdown本文には設定値を書き込まない。
