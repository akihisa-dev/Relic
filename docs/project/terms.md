...
| チャートタブ | Chart tab | `ChartTab`, `kind: "chart"`, `openChartInPane`, `chart-cards`, `chart-sphere`, `chart-chronicle` | `app/src/renderer/store/editorStore.ts`, `app/src/renderer/store/editorStoreModel.ts`, `app/src/renderer/hooks/useAppTabRenderers.tsx` | [design](../design/DESIGN.md) |
...
### ビュー
ユーザーが情報を見る目的ごとに切り替える、メインエリアの主要な表示単位。Relicの主要ビューは、Markdownを表示・編集する「ファイルビュー」、一覧から選んだ1件の代表画像を見る「カードビュー」、情報同士の関係を見る「グラフビュー」、同じ関係を3次元空間で見る「スフィアビュー」、情報を時間軸で見る「クロニクルビュー」の5つとする。ビューは仕様・設計・会話で使う概念名であり、実装上はファイルタブまたはチャートタブとしてメインエリアへ表示する。画面上では限られた表示幅で素早く識別できるよう、対応するUI名を「ファイル」「カード」「グラフ」「スフィア」「クロニクル」とする。

### ファイルビュー
Markdownファイルの内容を表示・編集する主要ビュー。UI名は「ファイル」。ファイルサイドバーで対象を選び、メインエリアのファイルタブへ表示する。ライブプレビューとソースモードは、ファイルビュー内のエディタ表示モードとして扱う。

### カードビュー
`card` 固定プロパティを持つMarkdownファイルを代表画像と表示名で一覧する主要ビュー。UI名は「カード」。メインエリアの `cards` チャートタブとして開く。
