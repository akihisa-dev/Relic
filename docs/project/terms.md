...
| チャートタブ | Chart tab | `ChartTab`, `kind: "chart"`, `openChartInPane`, `chart-chronicle` | `app/src/renderer/store/editorStore.ts`, `app/src/renderer/store/editorStoreModel.ts`, `app/src/renderer/hooks/useAppTabRenderers.tsx` | [design](../design/DESIGN.md) |
...
| 暦設定 | Calendar settings | `chronicleSettings`, `ChronicleSettingsPanel`, `ChronicleCalendarSettings`, `chronicleCalendars` | `app/src/renderer/components/ChronicleSettingsPanel.tsx`, `app/src/renderer/hooks/useWorkspaceChronicleCalendars.ts`, `app/src/shared/ipc/workspace.ts`, `app/src/main/settings/workspaceSettings.ts` | [frontmatter](../features/frontmatter.md), [data-model](../engineering/data-model.md), [design](../design/DESIGN.md) |
...
| 機能トグル | Feature toggles | `FeatureToggles`, `featureToggles`, `tools`, `frontmatter`, `graph`, `chronicleSettings`, `chronicle`, `rightPanelOutline`, `rightPanelLinks` | `app/src/shared/ipc.ts`, `app/src/main/settings/appSettings.ts`, `app/src/renderer/components/SettingsPanel.tsx` | [design](../design/DESIGN.md), [development](../development.md) |
...
### ビュー
ユーザーが情報を見る目的ごとに切り替える、メインエリアの主要な表示単位。Relicの主要ビューは、Markdownを表示・編集する「ファイルビュー」、情報同士の関係を見る「グラフビュー」、情報を時間軸で見る「クロニクルビュー」の3つとする。ビューは仕様・設計・会話で使う概念名であり、実装上はファイルタブまたはチャートタブとしてメインエリアへ表示する。画面上では限られた表示幅で素早く識別できるよう、対応するUI名を「ファイル」「グラフ」「クロニクル」とする。
...
### チャートタブ
グラフビューまたはクロニクルビューをメインエリア内に開くタブ。
...
### 暦設定
クロニクルビューで使うメイン暦とサブ暦を管理する現行のパネルタブ。暦名とサブ暦のメイン暦開始年を、ワークスペース設定の現行フィールド `chronicleCalendars` へ保存する。schemaVersion未指定の旧ワークスペース設定にある同名フィールドも互換的に読み込むが、互換読み込み専用の廃止済み機能ではない。Markdown本文には設定値を書き込まない。
...
### 機能トグル
設定パネルタブで切り替える機能のオン / オフ。現行実装では、ファイル加工ツール、フロントマター設定、暦設定、クロニクル、右パネルのアウトライン、右パネルのリンクの表示・操作入口を制御する。
