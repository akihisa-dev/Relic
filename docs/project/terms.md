...
### ビュー
ユーザーが情報を見る目的ごとに切り替える、メインエリアの主要な表示単位。Relicの主要ビューは、Markdownを表示・編集する「ファイルビュー」、代表画像で一覧する「カードビュー」、情報同士の関係を見る「グラフビュー」、同じ関係を3次元空間で見る「スフィアビュー」、情報を時間軸で見る「クロニクルビュー」の5つとする。ビューは仕様・設計・会話で使う概念名であり、実装上はファイルタブまたはチャートタブとしてメインエリアへ表示する。画面上では限られた表示幅で素早く識別できるよう、対応するUI名を「ファイル」「カード」「グラフ」「スフィア」「クロニクル」とする。
