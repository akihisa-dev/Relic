...
### ビュー
ユーザーが情報を見る目的ごとに切り替える、メインエリアの主要な表示単位。Relicの主要ビューは、Markdownを表示・編集する「ファイルビュー」、情報同士の関係を見る「グラフビュー」、情報を時間軸で見る「クロニクルビュー」の3つとする。ビューは利用目的の名称であり、実装上はファイルタブまたはチャートタブとしてメインエリアへ表示する。

### ファイルビュー
Markdownファイルの内容を表示・編集する主要ビュー。ファイルサイドバーで対象を選び、メインエリアのファイルタブへ表示する。ライブプレビューとソースモードは、ファイルビュー内のエディタ表示モードとして扱う。

### グラフビュー
ワークスペース内のMarkdown、タグ、添付画像、未解決リンクの関係をノードと線で表示する主要ビュー。メインエリアのチャートタブとして開く。

### クロニクルビュー
`chronicle` 固定プロパティを持つMarkdownファイルを時間軸上の年表として表示する主要ビュー。メインエリアのチャートタブとして開き、年表はクロニクルビュー内の可視化表現として扱う。
...
### 機能トグル
設定パネルタブで切り替える機能のオン / オフ。現行実装では、ファイル加工ツール、フロントマター設定、暦設定、クロニクルビュー、右パネルのアウトライン、右パネルのリンクの表示・操作入口を制御する。
