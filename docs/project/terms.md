...
| バブルビュー | Bubble view | `BubbleView`, `WorkspaceGraph`, `getWorkspaceGraphChannel`, `chartId: "graph"` | `app/src/renderer/components/BubbleView.tsx`, `app/src/renderer/bubble/`, `app/src/main/files/workspaceGraph.ts` | [navigation](../features/navigation.md), [data-model](../engineering/data-model.md), [design](../design/DESIGN.md) |
...
### ビュー
ユーザーが情報を見る目的ごとに切り替える、メインエリアの主要な表示単位。Relicの主要ビューは、Markdownを表示・編集する「ファイルビュー」、一覧から選んだ1件の代表画像を見る「カードビュー」、全Markdownファイルとプロパティを見る「テーブルビュー」、情報同士の関係を見る「バブルビュー」、同じ関係を3次元空間で見る「スフィアビュー」、情報を時間軸で見る「クロニクルビュー」の6つとする。ビューは仕様・設計・会話で使う概念名であり、実装上はファイルタブまたはチャートタブとしてメインエリアへ表示する。画面上では限られた表示幅で素早く識別できるよう、対応するUI名を「ファイル」「カード」「テーブル」「バブル」「スフィア」「クロニクル」とする。
...
### バブルビュー
共有ワークスペースグラフから実在するMarkdownファイル同士の関係だけをノードと線で表示する主要ビュー。タグ、添付画像、未解決リンクは共有データに含まれても表示しない。UI名は「バブル」。メインエリアの `graph` チャートタブとして開く。`graph` は保存済み設定とタブ識別の互換性を保つ内部IDであり、UIには表示しない。

### スフィアビュー
バブルビューと同じ関係データを、回転・拡大縮小できる3次元空間へ表示する正式な独立主要ビュー。UI名は「スフィア」。メインエリアの `sphere` チャートタブとして開き、2次元のバブルビューとは別の表示状態とライフサイクルを持つ。
