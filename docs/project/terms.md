...
| チャートタブ | Chart tab | `ChartTab`, `kind: "chart"`, `openChartInPane`, `chart-cards`, `chart-table`, `chart-sphere`, `chart-chronicle` | `app/src/renderer/store/editorStore.ts`, `app/src/renderer/store/editorStoreModel.ts`, `app/src/renderer/hooks/useAppTabRenderers.tsx` | [design](../design/DESIGN.md) |
...
### ビュー
ユーザーが情報を見る目的ごとに切り替える、メインエリアの主要な表示単位。Relicの主要ビューは、Markdownを表示・編集する「ファイルビュー」、一覧から選んだ1件の代表画像を見る「カードビュー」、全Markdownファイルとプロパティを見る「テーブルビュー」、情報同士の関係を見る「グラフビュー」、同じ関係を3次元空間で見る「スフィアビュー」、情報を時間軸で見る「クロニクルビュー」の6つとする。ビューは仕様・設計・会話で使う概念名であり、実装上はファイルタブまたはチャートタブとしてメインエリアへ表示する。画面上では限られた表示幅で素早く識別できるよう、対応するUI名を「ファイル」「カード」「テーブル」「グラフ」「スフィア」「クロニクル」とする。
...
### テーブルビュー
ワークスペース内のすべてのMarkdownファイルと、ユーザーが選んだトップレベルのフロントマタープロパティを読み取り専用の表で表示する主要ビュー。UI名は「テーブル」。メインエリアの `table` チャートタブとして開く。
...
### チャートタブ
カードビュー、テーブルビュー、グラフビュー、スフィアビュー、またはクロニクルビューをメインエリア内に開くタブ。
...
### 機能トグル
設定パネルタブで切り替える機能のオン / オフ。現行実装では、ファイル加工ツール、フロントマター設定、カード、テーブル、グラフ、スフィア、クロニクルの表示・操作入口を制御する。右パネルのアウトラインとリンクは常に利用可能であり、機能トグルの対象にしない。
