...
| 固定プロパティ | Fixed property | `FIXED_FIELDS`, `RESERVED_FIELD_NAMES`, `aliases`, `tags`, `status`, `chronicle0`〜`chronicle9` | `app/src/renderer/frontmatterSettingsModel.ts`, `app/src/main/ipc/workspaceHandlerValidators.ts` | [frontmatter](../features/frontmatter.md), [data-model](../engineering/data-model.md) |
...
### 固定プロパティ
Relicが意味を知っているフロントマターキー。`aliases`、`tags`、`status`、`chronicle0`〜`chronicle9` を指す。
