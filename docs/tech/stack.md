...
| YAML処理 | js-yaml | プロパティの読み書きに使用 |
| クラウド同期ディレクトリ対応 | ネイティブファイルシステム経由 | iCloud Drive / OneDrive / Dropbox などを、OS上のローカルディレクトリとして直接読み書き。各クラウドサービスのAPI連携は持たない |
| UIフレームワーク | React 19 | レンダラーUIを構築 |
| React状態管理 | Zustand | 横断的UI状態に限定して使用。カード内容・検索結果・設定永続化は入れない |
| 設定保存 | Electron userData 配下のJSON + 自前設定サービス | アプリ設定とカードブック設定を分けて保存 |
...
| テストフレームワーク | Vitest 3 | `vitest run`。設定は `app/vitest.config.ts` |
