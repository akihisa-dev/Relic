...
|------|-------------|------------|--------------|----------|
| カードブック | Cardbook | `CardbookSummary`, `CardbookState`, `cardbook:*`, `cardbooks`, `lastCardbookId` | `app/src/shared/ipc.ts`, `app/src/main/cardbook/cardbookService.ts`, `app/src/main/settings/appSettings.ts` | [card-management](../spec/card-management.md), [data-model](../architecture/data-model.md) |
| カード | Card | `MarkdownCardContent`, `CardbookCardNode`, `CardTab`, `kind: "card"` | `app/src/shared/ipc.ts`, `app/src/main/ipc/markdownCardHandlers.ts`, `app/src/renderer/store/editorStore.ts` | [card-management](../spec/card-management.md), [editor](../spec/editor.md), [markdown](../spec/markdown.md) |
| カードフォルダ | Card folder | `CardbookCardFolderNode`, `CreateCardFolderInput`, `RenameCardFolderInput`, `MoveCardFolderInput` | `app/src/shared/ipc.ts`, `app/src/main/ipc/cardFolderItemHandlers.ts`, `app/src/main/cards/cardFolders.ts` | [card-management](../spec/card-management.md) |
...
| 内部リンク | Internal link | `WikiLink`, `ResolvedWikiLink`, `resolveWikiLinks`, `buildWikiLinkCompletionSource` | `app/src/shared/links.ts`, `app/src/renderer/editorExtensions.ts`, `app/src/main/cards/linkUpdater.ts` | [links-and-tags](../spec/links-and-tags.md), [markdown](../spec/markdown.md) |
| バックリンク | Backlinks | `Backlink`, `getBacklinksChannel`, `readBacklinks`, `links.backlinks` | `app/src/shared/ipc.ts`, `app/src/main/cards/backlinks.ts`, `app/src/renderer/components/AppRightPanel.tsx` | [links-and-tags](../spec/links-and-tags.md) |
...
| タグ | Tag | `CardbookTagSummary`, `getCardbookTagsChannel`, `tags`, `frontmatterTags` | `app/src/shared/ipc.ts`, `app/src/main/cards/tags.ts`, `app/src/shared/tags.ts` | [links-and-tags](../spec/links-and-tags.md), [frontmatter](../spec/frontmatter.md) |
| プロパティ記述 | Properties block | `frontmatter`, `parseFrontmatter`, `writeFrontmatter`, `getFrontmatterValueCandidatesChannel` | `app/src/main/cards/frontmatter.ts`, `app/src/renderer/editorFrontmatter.ts`, `app/src/renderer/editorFrontmatterModel.ts` | [frontmatter](../spec/frontmatter.md), [data-model](../architecture/data-model.md) |
| プロパティテンプレート | Properties template | `FrontmatterTemplate`, `frontmatterTemplates`, `getFrontmatterTemplatesChannel` | `app/src/shared/ipc.ts`, `app/src/main/settings/appSettings.ts`, `app/src/main/ipc/cardbookPreferenceHandlers.ts` | [frontmatter](../spec/frontmatter.md) |
| 固定プロパティ | Fixed property | `FIXED_FIELDS`, `RESERVED_FIELD_NAMES`, `aliases`, `tags`, `status`, `timeline` | `app/src/renderer/frontmatterSettingsModel.ts`, `app/src/main/ipc/cardbookHandlerValidators.ts` | [frontmatter](../spec/frontmatter.md), [data-model](../architecture/data-model.md) |
...
| カラム | Column | `main-area`, `editor-layout`, `AppRail`, `AppCardsSidebar`, `AppRightPanel` | `app/src/renderer/components/AppEditorCardbook.tsx`, `app/src/renderer/components/AppRail.tsx`, `app/src/renderer/components/AppCardsSidebar.tsx` | [screens-macos](../ui/screens-macos.md), [DESIGN](../ui/DESIGN.md) |
...
| メインエリア | Main area | `main-area`, `AppEditorCardbook`, `PaneView`, `AppTopBar` | `app/src/renderer/components/AppEditorCardbook.tsx`, `app/src/renderer/components/AppTopBar.tsx`, `app/src/renderer/components/PaneView.tsx` | [screens-macos](../ui/screens-macos.md), [navigation](../ui/navigation.md) |
...
| カードサイドバー | Cards sidebar | `CardsSidebar`, `AppCardsSidebar`, `SidebarView: "cards"` | `app/src/renderer/components/CardsSidebar.tsx`, `app/src/renderer/components/AppCardsSidebar.tsx`, `app/src/renderer/store/uiStore.ts` | [card-management](../spec/card-management.md), [search](../spec/search.md), [screens-macos](../ui/screens-macos.md) |
| サイドバー | Sidebar | `SidebarView`, `isSidebarOpen`, `activeSidebarView` | `app/src/renderer/store/uiStore.ts`, `app/src/renderer/components/AppCardsSidebar.tsx`, `app/src/renderer/hooks/useSidebarResize.ts` | [navigation](../ui/navigation.md), [screens-macos](../ui/screens-macos.md) |
| カードブック切替 | Cardbook switcher | `RailCardbookSwitcher`, `switchCardbookChannel`, `SwitchCardbookInput`, `renameCardbookChannel` | `app/src/renderer/components/RailCardbookSwitcher.tsx`, `app/src/shared/ipc.ts`, `app/src/main/ipc/cardbookRegistrationHandlers.ts` | [card-management](../spec/card-management.md), [navigation](../ui/navigation.md) |
| カードツリー | Card tree | `CardTree`, `CardbookTreeNode`, `readCardbookCardTree`, `cardTree` | `app/src/renderer/components/CardTree.tsx`, `app/src/main/cards/cardTree.ts`, `app/src/shared/ipc.ts` | [card-management](../spec/card-management.md), [screens-macos](../ui/screens-macos.md) |
...
| エディタ本体 | Editor | `Editor`, `EditorView`, `buildExtensions`, `CardTab` | `app/src/renderer/components/Editor.tsx`, `app/src/renderer/editorExtensions.ts`, `app/src/renderer/store/editorStore.ts` | [editor](../spec/editor.md), [markdown](../spec/markdown.md), [editor-engine](../tech/editor-engine.md) |
...
| カードタブ | Card tab | `CardTab`, `kind: "card"`, `openCardInPane` | `app/src/renderer/store/editorStore.ts`, `app/src/renderer/store/editorStoreModel.ts`, `app/src/renderer/components/PaneView.tsx` | [editor](../spec/editor.md), [navigation](../ui/navigation.md) |
...
| Timelineタブ | Chart tab | `TimelineTab`, `kind: "timeline"`, `openTimelineChartInPane`, `timeline-charts`, `renderTimelineTab` | `app/src/renderer/store/editorStore.ts`, `app/src/renderer/store/editorStoreModel.ts`, `app/src/renderer/hooks/useAppTabRenderers.tsx` | [navigation](../ui/navigation.md), [screens-macos](../ui/screens-macos.md) |
| Timeline | Timeline | `timeline`, `TimelineChartSource`, `CardbookTimelineChart`, `defaultTimelineCharts`, `TimelineView`, `useCardbookTimeline` | `app/src/shared/ipc.ts`, `app/src/main/cards/timeline.ts`, `app/src/renderer/components/TimelineSidebar.tsx`, `app/src/renderer/timelineChartData.ts` | [frontmatter](../spec/frontmatter.md), [data-model](../architecture/data-model.md), [screens-macos](../ui/screens-macos.md) |
...
| カード加工ツール | Card tools | `tools`, `ToolsSidebar`, `tools:*`, `mergeCards`, `splitCardByHeading` | `app/src/shared/ipc.ts`, `app/src/main/ipc/toolActions.ts`, `app/src/renderer/components/ToolsSidebar.tsx` | [card-tools](../spec/card-tools.md), [screens-macos](../ui/screens-macos.md) |
...
| ピン留め | Pinning | `pinnedPaths`, `togglePinChannel`, `togglePin`, `cards.pinned` | `app/src/shared/ipc.ts`, `app/src/main/settings/cardbookSettings.ts`, `app/src/renderer/components/CardTree.tsx` | [card-management](../spec/card-management.md), [screens-macos](../ui/screens-macos.md) |
...
### プロパティ記述
カード先頭の `---` で囲まれたYAMLブロック。カードのプロパティ（タイトル・タグ・Timelineなど）を記述する場所。保存形式としてはプロパティを使うが、Relic上の概念としてはプロパティを主語にする。エディタ上では生のYAMLではなくフォーム形式で表示・編集できる。
...
### 固定プロパティ
Relicが意味を知っているプロパティキー。現時点では `aliases`、`tags`、`status`、`timeline` を指す。Timelineは `timeline` プロパティのみから生成する。
...
### ペイン
メインエリアの中を左右に分割した表示枠。分割表示中は左ペインと右ペインがそれぞれ独立したタブ列を持ち、カードタブ、パネルタブ、Timelineタブを開ける。右パネルはペインではない。
...
### メインエリア
画面右側の主要な作業領域。カードタブ、パネルタブ、Timelineタブを表示する。左右2ペイン分割に対応し、ペインごとに独立したタブ列を持つ。右パネルはメインエリアの右側に開く補助領域として扱う。
...
### 左レール
アプリ左端の縦アイコン領域。カードサイドバーの開閉、Timeline、カード加工、プロパティ設定、設定などの入口を持つ。登録済みカードブックがある場合は、レール下部にカードブック切替も表示する。
...
| Obsidian | VS Code |
|---|---|
| Vault switcher | Cardbook switcher |
...
### Timelineタブ
Timelineをメインエリア内に開くタブ。`timeline` プロパティから表示行を生成する。
...
### Timeline
`timeline` 固定プロパティから生成するTimeline表示。単年または期間を横軸の時系列上に表示し、バーの移動・伸縮でカードのプロパティへ書き戻す。
