...
- 方向性: `docs/journal/`、`docs/archive/`、完了済み・取り下げ済みの過去フェーズ文書は履歴として残し、現行実装へ合わせる編集対象から外す
- 実施: `docs/journal/` と `docs/archive/` の既存本文は書き換えず、`docs/INDEX.md` に履歴資料の読み方を追加した。`docs/_rules.md` には、履歴・アーカイブの扱い説明は索引、ルール、現在フェーズ正本へ書くこと、過去フェーズ文書は現在判断ではなく履歴として扱うこと、アーカイブ資料を現行実装に合わせて修正しないことを明記した
- 確認: `git diff --check` が通過した
- 残り: 次は `横断検証・P23締め更新` に進む

### 横断検証・P23締め更新

- 方向性: P23追加整理の最終単位として、現行正本、索引、参照語、実装との静的整合、検証コマンドをまとめて確認する
- 実施: 入口、product、spec、UI、architecture、tech、dev運用、履歴/アーカイブ扱いの各正本文書を、実装アプリと `app/package.json` / Electron Forge / Vite / Vitest構成へ合わせて整理した。`docs/journal/`、`docs/archive/`、完了済み・取り下げ済みの過去フェーズ文書は履歴として残し、現行判断に使わない扱いを `docs/INDEX.md` と `docs/_rules.md` に明記した
- 横断確認: `rg` で `AskUser`、`P24`、未実装ショートカット、古い画像表示方針、古いUIフォント、検索専用画面などの古い参照を確認した。現行正本側のMarkdownリンクは、記法例の `URL` / `path` / `images/image.png` を除外したうえで58ファイルを確認し、リンク切れなし
- 検証: `pnpm typecheck` が通過した。`pnpm test` が通過し、77ファイル、509件が成功した。`git diff --check` が通過した
- 完了判断: P23追加整理の10項目は完了。コード変更は行わず、文書側だけを現行実装へ合わせた
- 実アプリ未確認理由: 今回は文書整合整理であり、実アプリ操作と配布ビルドは基本範囲外とした。UI表示や保存形式を変更していないため、静的照合、型チェック、既存テストで完了判断した

### 用語集の英名・実装識別子対応整理（追加フォロー）

- 方向性: 用語集の日本語名だけではコード上の機能を追いにくいため、Relic英語名と実装識別子を分けて定義し、コード名を正式英名へ自動昇格させない
- 実施: `docs/product/glossary.md` に英名・実装識別子の扱いを追加し、現行の49用語すべてについて、Relic英語名、実装識別子、主な実装位置、関連仕様を対応表として整理した。計画上は43用語と見積もっていたが、現行ファイルには49項目あったため全項目を対象にした
- 確認: `rg` で主要識別子を確認した。表に載せた `app/src/` 配下の実装位置67件が実在することを確認した。Markdownリンク確認と `git diff --check` が通過した
- コード検証: 文書のみの変更でコード本体を編集していないため、`pnpm typecheck` と `pnpm test` は未実施

## 追加リファクタリング実施リスト

P23の追加リファクタリングとして、前回候補に挙げた5領域だけを対象にする。
P13は完了済み履歴として扱い、この追加作業では編集しない。
各単位は、対象差分とこのP23正本をまとめて確認し、日本語コミットを作る。
仕様変更、UI変更、保存形式変更、IPC/preload API変更、store状態構造変更は行わない。

1. [x] **追加リスト正本化**
   - この実施リスト、順番、完了条件をP23正本へ追記する。
   - 文書更新のみのため、`git diff --check` を最低確認とする。
2. [x] **Tool Actions IPC安全整理**
   - ファイル加工ツールのIPC入力validatorを追加し、workspace相対パスを安全に解決する。
   - IPCチャンネル名、preload API、戻り値、既存エラーコードは変更しない。
3. [x] **renderer CSS画面単位分割**
   - `styles.css` を画面単位CSSへ分ける。
   - class名、DOM構造、表示仕様は変更しない。
4. [x] **rendererテストfixture共通化**
   - `window.relic` mock、store reset、matchMedia setupを共通test utilityへ寄せる。
   - テスト期待値は変更しない。
5. [x] **shared IPC定義分割**
   - `shared/ipc.ts` を既存import経路を維持するfacadeにし、workspace/settings/tools/chartなどの型・channel定義を内部moduleへ分ける。
   - preload APIと既存renderer/main import経路は変更しない。
6. [x] **frontmatter model分割**
   - Frontmatter modelをフィールド定義・入力値変換とYAML保持シリアライズへ分離する。
   - 既存export経路とfrontmatter保存形式は変更しない。
7. [x] **追加リファクタリング締め確認**
   - `pnpm typecheck`、`pnpm test`、`git diff --check` を確認する。
   - P23正本に完了判断、残した未分割箇所、実アプリ未確認理由を記録する。

完了条件:

- 各実装単位で、対象テスト、影響範囲の既存回帰テスト、`pnpm typecheck`、`pnpm test`、`git diff --check` を確認する。
- 文書だけの単位では、`git diff --check` を最低確認とし、コードテストは不要と判断した理由を完了報告に書く。
- 実アプリ確認は、UI/仕様を変えない内部分離では原則未実施とし、自動テストで差分が出た場合だけ必要範囲を判断する。

### 追加リスト正本化

- 方向性: P23の追加リファクタリングは前回候補に挙げた5領域に限定し、完了済みのP13には追記しない
- 実施: `追加リファクタリング実施リスト` として、Tool Actions IPC安全整理、CSS分割、テストfixture共通化、shared IPC定義分割、frontmatter model分割、締め確認の順番と完了条件を追記した
- 確認: 文書更新のみのため、`git diff --check` を確認する。コードテストは不要
- 残り: 次は `Tool Actions IPC安全整理` に進む

### Tool Actions IPC安全整理

- 方向性: ファイル加工ツールのIPCチャンネル名、preload API、戻り値、既存エラーコードを変えず、IPC入力検証とworkspace相対パス解決を追加する
- 実施: `toolHandlerValidators.ts` を追加し、merge、split、title list、table of contentsのIPC入力をhandler側で検証するようにした。`resolveWorkspaceRelativePathOrRoot` を追加し、空の出力フォルダは従来どおりワークスペース直下として扱い、`..` や絶対パスは拒否するようにした。Tool Actionsのsource/target/output folderとoutput nameを実ファイルアクセス前に検証するようにした
- 確認: `pnpm exec vitest run src/main/ipc/toolHandlerValidators.test.ts src/main/files/paths.test.ts src/renderer/components/ToolsSidebar.test.tsx`、`pnpm typecheck`、`pnpm test`、`git diff --check` が通過した。全体テストは78ファイル、513件が通過した
- 残り: 次は `renderer CSS画面単位分割` に進む。実アプリ確認はIPC/preload APIとUIを変えない安全整理のため未実施

### renderer CSS画面単位分割

- 方向性: class名、DOM構造、表示仕様を変えず、巨大化していた `styles.css` を既存セクション境界に沿って画面・領域別CSSへ分割する
- 実施: `styles.css` をimport入口にし、base、shell/sidebar、file tree/search、workspace/editor、dashboard、preview/editor、right panel、settings、chronicle/graph、theme/motionの10ファイルへ機械的に分割した。分割ファイルを連結した内容が直前の `styles.css` と完全一致することを確認した
- 確認: `pnpm exec vitest run src/renderer/App.test.tsx src/renderer/components/ToolsSidebar.test.tsx src/renderer/components/DashboardPanel.test.tsx src/renderer/components/GraphSidebar.test.tsx src/renderer/components/ChronicleChartGrid.test.tsx src/renderer/components/Editor.test.tsx`、`pnpm typecheck`、`pnpm test`、`git diff --check` が通過した。全体テストは78ファイル、513件が通過した
- 残り: 次は `rendererテストfixture共通化` に進む。実アプリ確認はCSS宣言内容と適用順を維持した機械的分割のため未実施

### rendererテストfixture共通化

- 方向性: テスト期待値を変えず、rendererテストに散っていた `window.relic` mock、matchMedia setup、主要store resetを共通test utilityへ寄せる
- 実施: `src/test/rendererTestUtils.ts` を追加し、`makeRelicApi`、`installMatchMediaMock`、`resetRendererStores`、共通workspace stateを集約した。`App.test.tsx` と `ToolsSidebar.test.tsx` はこのutilityを使う形に整理した
- 確認: `pnpm exec vitest run src/renderer/App.test.tsx src/renderer/components/ToolsSidebar.test.tsx`、`pnpm typecheck`、`pnpm test`、`git diff --check` が通過した。全体テストは78ファイル、513件が通過した
- 残り: 次は `shared IPC定義分割` に進む。実アプリ確認はテストコードだけの責務整理のため未実施

### shared IPC定義分割

- 方向性: 既存の `../shared/ipc` import経路、preload API、IPCチャンネル名、型名を変えず、肥大化していた共有IPC定義をdomain別moduleへ分ける
- 実施: `ipc.ts` を再export facadeにし、チャンネル定数、tool入力、chart型、workspace/file/search型、settings型、`RelicApi` をそれぞれ `ipcChannels.ts`、`ipcTools.ts`、`ipcCharts.ts`、`ipcWorkspace.ts`、`ipcSettings.ts`、`ipcApi.ts` へ分割した
- 確認: `pnpm exec vitest run src/main/ipc/toolHandlerValidators.test.ts src/main/ipc/fileHandlerValidators.test.ts src/main/ipc/workspaceHandlerValidators.test.ts src/renderer/App.test.tsx`、`pnpm typecheck`、`pnpm test`、`git diff --check` が通過した。全体テストは78ファイル、513件が通過した
- 残り: 次は `frontmatter model分割` に進む。実アプリ確認は型定義とexport経路だけの分割のため未実施

### frontmatter model分割

- 方向性: 既存の `editorFrontmatterModel.ts` import経路、frontmatter保存形式、YAML保持挙動を変えず、フィールド定義・入力値変換とYAML保持シリアライズを分離する
- 実施: `editorFrontmatterModel.ts` を再export facadeにし、固定/カスタムフィールド判定、入力値変換、frontmatter dialog eventを `editorFrontmatterFields.ts` へ、YAML field解析、コメント/クォート保持、serialize、frontmatter block検出を `editorFrontmatterYaml.ts` へ分割した
- 確認: `pnpm exec vitest run src/renderer/editorFrontmatterModel.test.ts src/renderer/components/Editor.test.tsx`、`pnpm typecheck`、`pnpm test`、`git diff --check` が通過した。全体テストは78ファイル、513件が通過した
- 残り: 次は `追加リファクタリング締め確認` に進む。実アプリ確認はfrontmatter保存形式と既存export経路を維持した内部module分割のため未実施

### 追加リファクタリング締め確認

- 方向性: P23追加リファクタリング7項目を完了扱いにできるか、大型ファイル一覧、自動テスト、型検査、diff whitespaceで最終確認する
- 実施: 追加リファクタリング実施リストの7項目をすべて完了にした。大型ファイル一覧は `rg --files app/src | xargs wc -l | sort -nr | head -n 40` で確認した
- 確認: `pnpm typecheck`、`pnpm test`、`git diff --check` が通過した。全体テストは78ファイル、513件が通過した
- 完了判断: 前回候補に挙げた5領域は完了。IPCチャンネル名、preload API、戻り値、既存エラーコード、CSS class名、DOM構造、表示仕様、既存import/export経路、frontmatter保存形式、store状態構造は変更していない
- 残した意図的な未分割箇所: `App.test.tsx` と `Editor.test.tsx` は回帰観点のまとまりを優先し、今回のfixture共通化以上には分割しない。`chronicle-graph.css`、`preview-editor.css`、`right-panel.css` は画面・領域単位CSSとして残し、class適用順を崩す細分化は行わない。`App.tsx` はcomposition rootとして残す。`toolActions.ts`、`editorFrontmatterWidgetInputs.ts`、`chronicleTimelineAxis.ts` などは、現時点では純粋処理または単一責務に寄っており、今回の5領域追加リファクタリング対象からは外す
- 実アプリ未確認理由: 今回はIPC入力検証、CSS import経路、テストfixture、共有型facade、frontmatter内部moduleの整理であり、UI表示、保存形式、IPC/preload APIを変更していない。対象テスト、型検査、全体テスト、diff whitespaceで差分確認できたため、実アプリ確認は未実施
