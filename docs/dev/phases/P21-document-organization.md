...
- 方向性: `docs/_rules.md` は、`AI.md` とP21運用に合わせて、日誌を自動更新するように読めない表現へ整理する。コード変更、仕様変更、フェーズ変更は含めない
- 実施: `docs/_rules.md` の開発日誌ルールから、作業区切りや1日の終わりに自動記録する前提を外し、ユーザーが開発の区切り・終了または日誌更新を明示した場合だけ記録する形へ整理した
- 確認: `rg` で `docs/_rules.md` に `作業の区切りごと`, `1日の終わり` が残っていないことを確認した
- 残り: 今回範囲の文書管理ルール整理は完了。`docs/dev/phases.md` と `docs/journal/` はフェーズ区切り指示がないため更新しない

### 終了時の日誌更新漏れ防止

- 方向性: 日誌更新を終了後の別作業として扱わず、終了返答前の作業フローに組み込む。P21完了や次フェーズ移行は含めない
- 実施: `AI.md` と `docs/_rules.md` に、ユーザーが `終了`、`ここまで`、`区切り` などを明示した場合は、終了返答より先に日誌更新判断と必要な追記を行うことを明記した
- 実施: `docs/journal/2026-05-14.md` に、P21後半の文書整理、AI運用ルール整理、補助スクリプト/README整合、P21完了前チェック、今回の終了時日誌更新漏れ防止ルール追加を追記した
- 確認: `tail docs/journal/2026-05-14.md`, `rg "終了|日誌|区切り" AI.md docs/_rules.md`, `git diff --check` で確認した
- 残り: 今回範囲の日誌更新漏れ修正は完了。`docs/dev/phases.md` はP21完了や次フェーズ移行の明示がないため更新しない
...
- 完了: P21で扱った文書整理は完了とする
- 確認: 未コミット差分がないこと、主要文書に明確な古い前提が残っていないこと、日誌・アーカイブと記法例を除いたMarkdownリンク先が存在することを確認済み
- 残り: 次フェーズは未指定。ユーザーが次フェーズを明示するまで `docs/dev/phases.md` の `current` は `P21` のままにする
