...
| `app/src/main/` | Electronメインプロセス。ファイル操作・設定保存・検索インデックス・IPCハンドラ |
...
| `app/src/renderer/features/` | ファイルツリー・検索・設定・エディタなど機能単位のUI |
...
- メインプロセスは、ファイルシステム・検索インデックス・アプリ設定の責務を持つ
...
- ファイル内容・検索インデックス・LinkIndex / TagIndex・設定の永続化はZustandに入れない
...
- 外部URL画像は表示対象外にする
- 添付画像はMarkdown本文から参照されるワークスペース内相対パスの補助ファイルとして扱い、画像管理機能を作らない
...
- Markdown本文から参照される画像はワークスペース内相対パスの通常ファイル参照として扱い、専用添付フォルダを前提にしない
...
- MarkdownプレビューはHTMLをサニタイズし、画像読み込みはワークスペース内相対パスに限定する
...
- 機能トグルは料金モデルと結びつけない
- 機能トグル対象はファイル加工ツール / フロントマター / 右パネルに限定する
