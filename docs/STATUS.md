...
| [mockups/](mockups/) | 🔶 | 現行仕様に追従する静的画面モックあり（メイン・検索・Git・設定・初回起動）。実動UIではない |

---

## 開発（dev/）

実際のアプリ実装は凍結中。ユーザーが明示的に実装凍結解除を指示するまで、コード作成・プロジェクト雛形作成・依存関係導入・ビルド設定作成は行わない。
...
- **実装状態**：実際のアプリ実装は凍結中。ドキュメント整備のみ継続可能
