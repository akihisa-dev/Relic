...
- E2E、実アプリ操作、スクリーンショット、起動スモークは、ローカル作業ではユーザーがその作業で明示的に指示した場合だけ実行する
- テストは実ユーザーのワークスペースや外部サービスを使わない
- テスト層ごとの責務、主要利用経路、重複と不足の判断、E2E候補は [engineering/test-strategy.md](engineering/test-strategy.md) を正とする
...
| UIだけの軽微な見た目調整 | 型チェック、関連する既存テスト。表示確認はユーザーの明示指示時だけ |
