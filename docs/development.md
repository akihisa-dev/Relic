...
コミットのtypeだけからMAJOR、MINOR、PATCHを自動決定しない。

- PATCH: バグ修正、文書整理、テスト、内部整理、軽微なUI調整、開発環境や依存関係の軽微な変更
- MINOR: 機能追加、まとまった仕様・UI・文書構成の変更など。ユーザーが明示した場合だけ使用する
- MAJOR: 互換性や移行へ大きく影響する変更、正式版の区切りなど。ユーザーが明示した場合だけ使用する

`1.0.0` 未満では、ユーザーが明示した場合に限り、大きな互換性変更を `0.x.0` のMINOR更新として扱ってよい。
明示がなく迷う場合はPATCHを選ぶ。
