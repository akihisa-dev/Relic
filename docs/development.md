...
- ファイルツリーとクイックスイッチャー起動はファイルサイドバー内、設定や補助機能はパネルタブとして扱う
- UI状態と永続化データを混ぜない
...
- OSクリップボードの読み取りは、ブラウザ/エディタの通常pasteイベント、またはエディタ右クリックメニューの貼り付けなどユーザー操作に紐づく経路に限定する。rendererから任意タイミングで読み取れる汎用IPC APIは公開しない
...
- ただし、ユーザーがGitタグ作成を指示した場合は、リリース前確認に必要な `pnpm audit --prod` の実行許可を含む指示として扱う

### 秘密情報検知

- ローカルpush前は、`.githooks/secret-guard.sh` で秘密情報らしい差分を検知する
- `.githooks/pre-push` は `.githooks/secret-guard.sh --pre-push` を呼び出す
...
- npm / pnpm 依存関係とGitHub Actionsの更新確認に、GitHub上の依存更新Pull RequestやDependabotを使わない
- `.github/dependabot.yml` は作成しない
- 依存関係の更新確認は、必要なタイミングでローカルの `app/` から実行する
- ユーザーがGitタグ作成を指示した場合は、タグ作成前に `app/` で `pnpm outdated` と `pnpm audit --prod` を実行して依存状態を確認する
- ユーザーがGitタグ作成を指示した場合は、`pnpm audit --prod` について追加確認せず、リリース前確認の一部として実行する
- 更新候補がある場合は、影響の小さいものからローカルで更新し、検証に通ったものだけ採用する
- 更新で検証が壊れた場合はその更新を戻し、見送った理由をコミット本文または完了報告に記録する
- 依存更新では、`pnpm.overrides` の対象がまだ必要かを確認し、不要になった固定は同じコミットまたは別作業で外す
...
- 依存更新では、`app/` で `pnpm verify` と `pnpm licenses:check` を実行し、リポジトリルートで `git diff --check` を実行する
- ビルド基盤やElectronなど配布成果物に影響する依存を更新した場合は、`app/` で `pnpm package` または対象OSの安全ビルドコマンドを実行する
- Gitタグ作成指示以外で外部 registry へ依存情報を送る監査コマンドを実行する場合は、既存方針どおり事前にユーザーの明示許可を得る
...
- 通常のコード変更では `pnpm verify` を基本確認とし、型チェックとテストをまとめて実行する
- 仕様、UI、実装の変更を含む場合は、少なくとも型チェックと関連テストを実施し、必要に応じて `pnpm test` または `pnpm verify` を実行する
- 変更内容に対して `pnpm verify` が過剰な確認になる場合でも、テストを省略せず、対象テスト、型チェック、文書確認、差分確認など、変更内容に対応する確認へ絞る
- ユーザーに確認するのは、検証に外部アクセス、実ユーザーデータ、長時間の実行、配布成果物、実アプリ操作などの負担やリスクがある場合に限る
- 機能確認や不具合修正では、対象機能の仕様リスクに応じて型チェック、既存テスト、対象テストを実施する
...
- E2Eテストや配布ビルド確認は通常の開発では必須にしない。必要な場合だけ対象、負担、リスクを確認して実施する
- 実施した検証は完了報告、Issue、Pull Request、またはコミット本文で確認できるようにする
- 実施しなかった検証がある場合は、対象外にした理由を明記する
...
5. `app/` で `pnpm outdated` と `pnpm audit --prod` を実行し、依存状態を確認する
6. `app/` で `pnpm verify` を実行して成功することを確認する
7. リポジトリルートで `git diff --check` を実行して成功することを確認する
...
外部actionはGitHub公式actionを優先し、参照更新はDependabotではなくタグ作成前の手元確認で判断する。
