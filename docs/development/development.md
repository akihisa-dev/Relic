...
次の中から1つだけ選ぶ。
表にないカテゴリをその場で作らない。
複数の種類を含む変更では、そのコミットの主目的に最も近いカテゴリを選ぶ。

| カテゴリ | 使う場面 |
|----------|----------|
| `追加` | 新しい機能、文書、テスト、設定、ファイルを加える |
| `変更` | 既存の仕様、挙動、UI、ルール、構成を意図して変える |
| `修正` | 不具合、誤記、参照切れ、テスト失敗、仕様不一致を直す |
| `削除` | 不要または廃止した機能、文書、コード、設定をなくす |
| `整理` | 振る舞いを変えずに、構成、命名、重複、責務を整える |
| `更新` | 依存関係、バージョン、生成物、固定データを新しい状態にする |
| `検証` | テスト、チェック、診断、CI確認の仕組みを整える |
| `公開` | 配布ビルド、タグ、Release、公開作業を行う |

例:

```text
追加: コミット件名のカテゴリを追加
修正: ファイル移動時のリンク更新漏れを修正
変更: 設定パネルの操作導線を変更
```
