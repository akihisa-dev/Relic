...
この文書では、Relicの開発作業における作業範囲、文書管理、実装規約、検証、テスト、コミット、Issue対応、バージョン更新、リリースの扱いを定義する。
...
- 各問いには `関連:` で由来となる文書を明記する
- 決定したら、決定内容を該当する仕様、設計、開発ルールの文書へ反映する
- 反映済みの問いは [questions.md](questions.md) に残さない
- 作業履歴を文書内に蓄積しない
...
- 未決定事項がある場合は実装で推定せず、[questions.md](questions.md) に追加する
...
`type` は英語小文字の行動語にする。
Conventional Commitsの `type: description` 形式を基本にしつつ、Relicでは対象名ではなく行動で分類する。
次の中から1つだけ選ぶ。
表にない `type` をその場で作らない。
複数の種類を含む変更では、そのコミットの主目的に最も近い `type` を選ぶ。

| type | 使う場面 |
|------|----------|
| `add` | 新しい機能、文書、テスト、設定、ファイルを加える |
| `change` | 既存の仕様、挙動、UI、ルール、構成を意図して変える |
| `fix` | 不具合、誤記、参照切れ、テスト失敗、仕様不一致を直す |
| `remove` | 不要または廃止した機能、文書、コード、設定をなくす |
| `refactor` | 振る舞いを変えずに、構成、命名、重複、責務を整える |
| `update` | 依存関係、バージョン、生成物、固定データを新しい状態にする |
| `test` | テスト、チェック、診断、CI確認の仕組みを整える |
| `release` | 配布ビルド、タグ、Release、公開作業を行う |
...
- リリース、配布ビルド、Gitタグ作成、またはユーザーが明示した区切りでバージョンを上げる
