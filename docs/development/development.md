...
| カテゴリ | 使う場面 |
|----------|----------|
| `文書` | ドキュメント、README、開発ルール、索引の追加・修正・削除 |
| `機能` | ユーザーから見える機能追加 |
| `修正` | 不具合修正、仕様どおり動いていない挙動の修正 |
| `UI` | 画面構成、表示、操作導線、見た目の調整 |
| `整理` | 振る舞いを変えないリファクタリング、構成整理、重複削除 |
| `テスト` | テストの追加、修正、検証補助の整理 |
| `開発` | ビルド、依存関係、スクリプト、開発環境、CI設定 |
| `リリース` | バージョン更新、タグ、配布ビルド、Release運用 |
| `安全` | セキュリティ、秘密情報、権限、脆弱性対応 |
