...
- Draft Release workflowが受け付けるタグ名も `vMAJOR.MINOR.PATCH` 形式だけとする
...
タグ名が `vMAJOR.MINOR.PATCH` 形式ではない場合も、workflowはビルド前に失敗する。
...
2. タグ名が `vMAJOR.MINOR.PATCH` 形式であり、Gitタグから実行されていることを確認する
