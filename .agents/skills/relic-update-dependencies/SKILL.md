...
description: Relicのnpm・pnpm依存関係とGitHub Actionsを候補確認から段階的に更新し、互換性、既知リスク、ライセンス、THIRD_PARTY_NOTICES、SBOM、テスト、bundle、配布物まで安全に整合させる。依存更新、Electron・Vite・Reactなどの更新、pnpm audit対応、outdated確認、override整理、ロックファイル更新の依頼に使う。確認だけでは読み取り専用とし、リリース前確認だけはrelic-releaseを優先する。
---
...
4. production dependency、devDependency、GitHub Actions、`pnpm.overrides` を分ける。production dependencyの追加は既存依存で目的を満たせない場合だけ行う。
...
3. major更新、Electron、Vite、React、保存・parse・sanitizeに関わる依存では、公式release notes、migration guide、配布パッケージの情報を一次情報として互換性を確認する。
...
2. `pnpm licenses:check`、`pnpm verify`、`pnpm docs:index:check`、リポジトリルートの `git diff --check` を実行する。
3. rendererの初期読込、遅延読込、CSS、assetへ影響する場合は `pnpm build:size:check` を実行する。
4. Electron、Forge、Vite、native moduleなど配布成果物へ影響する場合は、対象OSの現行safe buildまたはpackage検査を選ぶ。未確認のOSを確認済みと報告しない。
...
採用した更新、見送った候補と理由、互換対応、ライセンス・SBOM、audit、テスト、bundle、対象OS確認、未実施項目を分けて報告する。push、タグ、Releaseは明示指示がある場合だけ行う。
