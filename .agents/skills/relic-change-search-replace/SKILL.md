...
description: Relicの全文・ファイル名・タグ・フロントマター検索と、単一・一括置換、正規表現、プレビュー、競合検知、要求順序を追加・修正する。検索精度、検索上限、旧IPC入力互換、置換安全性、検索結果状態の変更に使う。共有ワークスペース索引は利用して変更pathの再読込を要求するが、索引構造・解析キャッシュ・バックリンク・タグ派生の変更はrelic-change-links-index、クイックスイッチャーはrelic-change-navigation、ファイル書込一般はrelic-change-workspace-filesを優先または併用する。
---
...
2. `git status --short` を確認し、検索種別、入力互換、要求制御、結果表示、単一置換、プレビュー、一括置換のどこを変えるか限定する。
...
2. 文字列単体、旧配列、旧query・mode keyはIPC互換アダプタだけで現行構造へ正規化する。
3. 検索語句、検索種別、fieldを安全に正規化できない場合はmain検索を呼び出さない。
4. 互換入力の分岐を検索service、索引、Rendererへ広げず、削除する場合は保存済み呼出元と契約versionの根拠を確認する。
...
3. 現行IPC入力と全互換入力、不正型、正規化不能値、handler未実行を確認する。
...
変更した検索種別、入力互換、置換安全条件、索引consumer契約、更新した正本、テスト、性能確認、未確認項目を報告する。コミット時は `$relic-commit` に従う。
