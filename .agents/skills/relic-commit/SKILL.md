...
7. 対象パスまたはhunkを明示してステージする。`git add .` と `git add -A` を使わない。`git diff --cached --check`、`git diff --cached`、`.githooks/pre-commit` を明示実行し、hook設定の有無に依存せず対象、機密情報、意図しない差分を再確認する。stage-onlyはこの結果を報告して終了する。
