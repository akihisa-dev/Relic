...
3. フォーム、ソースYAML、固定プロパティ設定、検索・タグ・クロニクルへの派生のどこを変えるかを特定する。
...
11. プロパティのドラッグ中はMarkdownを変更せず、確定したdrop時だけトップレベルYAMLの順序を書き戻す。pointer cancel、lost capture、Escape、元位置へのdropでは順序とMarkdownを変更せず、次の操作へ一時状態を残さない。
12. 並べ替えるキーに属する複数行値とインラインコメントはキーと一緒に移動する。独立したコメント行、クォート、ブロックスカラー、キー間の空行、改行コード、本文との境界を保持し、全体dumpで整形しない。

## 検証する

1. parseとserializeには未設定、空、正常、不正、未完了、scalar、array、mappingの単体テストを追加する。
2. 局所更新には複数行値、隣接キー、独立・インラインコメント、単純スカラーのクォート、ブロックスカラー、空行、LF・CRLF、未知キー、最後のキー削除の往復テストを追加する。
3. フォームには追加、編集、配列追加、削除、不正入力、折りたたみ、再作成に加え、drop確定、元位置drop、pointer cancel、lost capture、Escape後のrendererテストを追加する。中断時はMarkdownのbyte列と順序が変わらず、その後の並べ替えを開始できることを確認する。
4. 派生利用へ触れた場合はタグ、別名、検索候補、クロニクルの該当テストも実行する。
5. parser、serializer、仕様分岐を追加・変更した場合は対象テストまたは回帰テストを追加し、`app/` で `pnpm verify` を実行する。その他の変更でも対象node・rendererテストと `pnpm typecheck` を実行し、影響が広い場合は `pnpm verify` を実行する。
