...
5. `.githooks/secret-guard.sh --range <outgoing-range>` と `app/` の `pnpm version:check -- <base> <head>` を明示実行する。`core.hooksPath` の設定有無に依存せず、送信対象の秘密情報、コミットversion、SBOM同期を確認し、いずれかが失敗した場合はpushしない。
