...
description: Relicのスフィアビューについて、共有グラフデータからの3次元派生、WebGL runtime、球状配置、星の等級・星座強調・ガイド、カメラの周回・平行移動・zoom、選択、遅延読込、待機停止、context lossを追加・修正する。スフィア固有の表示、視点、操作、性能、WebGL障害に使う。共有リンク索引とグラフデータ契約はrelic-change-links-index、2次元グラフのCanvas・物理演算はrelic-change-graph、機能トグルと保存設定はrelic-change-settings、タブ遷移はrelic-change-navigation、見た目だけはrelic-change-uiを優先または併用する。
---
...
1. グラフビューと同じ取得hook・更新番号・cacheを使い、スフィア専用の第二索引や第二IPCを作らない。
2. 共有node・edgeを複製して3次元libraryへ渡し、libraryが付加する座標や内部状態を共有データへ書き戻さない。
3. link・aliases・tags・attachmentの解析と共有索引は `$relic-change-links-index`、2次元グラフ固有のscene・Canvas・Workerは `$relic-change-graph` に委ねる。
...
5. WebGLを利用できない場合、生成に失敗した場合、contextを失った場合はスフィアタブ内だけを失敗状態にし、2次元グラフとアプリ全体を巻き込まない。
