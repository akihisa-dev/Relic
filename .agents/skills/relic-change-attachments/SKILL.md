...
6. 添付画像のリンク解析・グラフノード化を変える場合、形式と安全なpathは本Skill、リンク構文は `$relic-change-links-index`、派生グラフは `$relic-change-bubble` が所有する。
