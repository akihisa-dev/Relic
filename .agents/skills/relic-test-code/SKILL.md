...
3. `docs/development.md` の「検証とテスト」、`app/vitest.config.ts`、`app/package.json` の現行scriptを確認する。
...
3. 原則として対象実装の近くへ `*.test.ts` または `*.test.tsx` を置き、複数モジュールをまたぐ統合用の共通資源だけを `app/src/test/` へ置く。
...
5. 失敗を解消するためにassertion、coverage threshold、安全条件を弱めない。仕様とtestが不一致なら正本と現行実装を確認して正しい側を直す。
6. `git diff --check` と全差分を確認し、一時データ、coverage生成物、debug log、機密情報が残っていないことを確かめる。
