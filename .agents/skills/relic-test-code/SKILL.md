...
description: RelicのVitest・React Testing Libraryによる自動テストについて、失敗の再現と原因切り分け、回帰テストの追加・修正、Node・jsdom環境の選択、fixture・mock・一時データの安全性、coverage結果の診断まで扱う。ローカルのテスト失敗、flaky test、テスト追加、回帰防止、coverage不足の調査や修正に使う。機能本体の変更は対応するrelic-change系Skill、実画面確認はrelic-test-development-app、GitHub Actions上だけの失敗はgithub:gh-fix-ciを優先する。
---
...
5. テスト群の役割監査や配置変更では `pnpm test:inventory` を実行し、Electron実行・macOS packageをVitest件数へ混ぜず、不足する失敗責務を示す。
6. 失敗を解消するためにassertionや安全条件を弱めない。coverage割合そのものを合否条件にせず、未実行の重要分岐があれば具体的な失敗経路をテストする。仕様とtestが不一致なら正本と現行実装を確認して正しい側を直す。
