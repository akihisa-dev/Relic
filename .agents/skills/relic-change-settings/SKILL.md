...
description: Relicのアプリ設定・ワークスペース別設定について、schema、現行version限定の読込、既定値、入力検証、安全書き込み、設定ファイル単位の更新queue、IPC、設定パネルの保存挙動を変更する。テーマ、表示言語、エディタ設定、機能トグル、登録ワークスペース、ピン留め、チャート・テーブル・フロントマター候補、同時更新・保存失敗などの永続化追加・修正に使う。各ビュー固有の設定操作はrelic-change-table-view・relic-change-bubble・relic-change-sphere・relic-change-chronicle、翻訳はrelic-change-localization、実ファイル操作はrelic-change-workspace-files、IPC境界はrelic-change-electron-boundariesを優先または併用する。
---
...
2. `git status --short` を確認し、新しい値、既存値変更、version拒否、保存障害、設定UIのどれを扱うか限定する。
...
4. 読込は現行 `schemaVersion` のJSON objectだけを受理する。version欠落、旧version、未知の将来versionは拒否し、元ファイルを上書き、退避、現行形式へ暗黙変換しない。
5. 現行schema内の不正項目を安全側へ正規化する処理と、未対応version全体の拒否を混同しない。
6. schema変更が必要な場合は、version、互換性、既存ファイルの扱い、正本文書を先に決める。version欠落を0と仮定したり、移行を既定方針として追加したりしない。
7. 保存時は現行キーだけを書き込み、拒否した設定ファイルの内容を別の初期値で置き換えない。
8. 削除する設定値は既存利用者への影響、既定値、UI、文書、version方針を確認してから除去する。
...
4. 読込不能や未対応versionがアプリ全体を不意に停止させないよう、呼出側の失敗処理まで確認する。
...
2. 現行schemaの読込・保存、version欠落、旧version、将来versionの拒否、拒否時の原文不変、壊れたJSONだけの退避を確認する。
...
5. schema parser、version拒否・保存の仕様分岐、ファイル操作、IPC入力検証を追加・変更した場合は対象テストまたは回帰テストを追加し、`app/` で `pnpm verify` を実行する。その他の変更でも対象のnode・rendererテストと `pnpm typecheck` を実行し、必要なら `pnpm architecture:check` または `pnpm verify` を実行する。
