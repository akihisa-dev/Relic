...
- `userData` 配下の設定ファイルと再生成可能なindex cacheは、ディレクトリを `0700`、ファイルを `0600` 相当に制限する。
...
- テキストとして扱える全パスを検査し、GitHub token、provider token、認証情報の代入、秘密鍵（PEMを含む）、認証付きDB接続文字列を検知する。guard用スクリプトやworkflowも検査対象から除外しない。
...
- Action参照は変更可能なtagやbranchではなく40桁のcommit SHAへ固定する。Draft Releaseの書き込みjobは保護された`release` environmentを使い、tag ref、`app/package.json`のversion、タグ名の一致を確認してからReleaseを更新する。
