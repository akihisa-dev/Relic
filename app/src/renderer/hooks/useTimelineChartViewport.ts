...
interface UseTimelineChartViewportInput {
...
export interface TimelineChartViewport {
...
export function useTimelineChartViewport({
  activeChart,
  activeSource,
  axisEnd,
  axisStart,
  ...
  nameColumnWidth,
  unitWidth
}: UseTimelineChartViewportInput): TimelineChartViewport {
  ...
  const initialTimelineScrollKeyRef = useRef<string | null>(null);
  ...
  const scrollToToday = useCallback((): void => {
    const target = timelineNavigationTarget(entries, axisStart, axisEnd);
    if (target !== null) scrollToTimelineValue(target);
  }, [axisEnd, axisStart, entries, scrollToTimelineValue]);

  const scrollToTimelineFocus = useCallback((): void => {
    const target = timelineNavigationTarget(entries, axisStart, axisEnd);
    if (target !== null) scrollToTimelineValue(target);
  }, [axisEnd, axisStart, entries, scrollToTimelineValue]);
  ...
  const startChartPan = useCallback((event: PointerEvent<HTMLDivElement>): void => {
    ...
    if (event.target instanceof Element && event.target.closest(".timeline-fill, .timeline-card-name, .timeline-offscreen-jump, .timeline-vertical-offscreen-jump, .timeline-vertical-minimap")) return;
  ...
  useLayoutEffect(() => {
    if (activeSource !== "timeline" || !activeChart) return;

    const key = activeChart.id;
    if (initialTimelineScrollKeyRef.current === key) return;

    initialTimelineScrollKeyRef.current = key;
    scrollToTimelineFocus();
  }, [activeChart, activeSource, scrollToTimelineFocus]);
