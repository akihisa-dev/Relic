...
import type { TimelineChartEntry, TimelineChartEntryEditKind, TimelineChartSource, UpdateTimelineChartEntryInput } from "../../shared/ipc";
import {
  createAdaptiveTimelinePointerDelta,
  type DragPreview
} from "../timelineTimeline";

interface UseTimelineEntryDragInput {
...
export interface TimelineEntryDrag {
...
export function useTimelineEntryDrag({
  activeSource,
  onUpdateEntry,
  resetKey,
  unitWidth
}: UseTimelineEntryDragInput): TimelineEntryDrag {
  ...
  const startEntryEdit = useCallback((
    ...
    const dragDelta = createAdaptiveTimelinePointerDelta(startClientX, unitWidth, event.timeStamp);
