...
import { useTimelineChartViewport } from "./useTimelineChartViewport";
...
function chart(source: TimelineChartSource = "timeline"): CardbookTimelineChart {
  return {
    entries: [entry()],
    id: source,
    name: source,
...
function renderViewport(activeSource: TimelineChartSource = "timeline") {
  const hook = renderHook(() => useTimelineChartViewport({
...
describe("useTimelineChartViewport", () => {
  ...
  it("barやcard名上のpointer downではpanを開始しない", () => {
    const { hook } = renderViewport();
    const chartElement = makeDiv({ clientWidth: 200, scrollWidth: 1000 });
    const excludedTarget = document.createElement("button");
    excludedTarget.className = "timeline-fill";
