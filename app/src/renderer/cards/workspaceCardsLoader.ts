...
export interface WorkspaceCardsRequest {
  revision: number;
  workspaceId: string;
}

interface CacheEntry {
  promise: Promise<RelicResult<WorkspaceCard[]>>;
  status: "pending" | "success";
}

const maximumCachedWorkspaceCardLists = 2;
const cache = new Map<string, CacheEntry>();
let cacheClient: RelicClient | undefined;

export function loadWorkspaceCards(request: WorkspaceCardsRequest): Promise<RelicResult<WorkspaceCard[]>> {
  ...
  const entry: CacheEntry = { promise: client.getWorkspaceCards(), status: "pending" };
  touch(key, entry);
...
export function resetWorkspaceCardsCache(): void {
  cache.clear();
