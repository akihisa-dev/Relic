import { useEffect, useMemo, useRef, useState } from "react";
import type { ReactElement } from "react";

import type { SearchMode } from "../../shared/ipc";
import {
  activeCardSearchModeLabel,
  cardSearchModeOptions,
  frontmatterValueCandidatesForField
} from "../cardsSidebarModel";
import { useT } from "../i18n";

interface CardsSidebarSearchProps {
  onSearchFrontmatterFieldChange: (field: string) => void;
  onSearchModeChange: (mode: SearchMode) => void;
  onSearchQueryChange: (query: string) => void;
  searchError: string | null;
  searchFocusRequest: number;
  searchFrontmatterCandidates: Record<string, string[]>;
  searchFrontmatterField: string;
  searchFrontmatterFields: string[];
  searchMode: SearchMode;
  searchQuery: string;
}

export function CardsSidebarSearch({
  onSearchFrontmatterFieldChange,
  onSearchModeChange,
  onSearchQueryChange,
  searchError,
  searchFocusRequest,
  searchFrontmatterCandidates,
  searchFrontmatterField,
  searchFrontmatterFields,
  searchMode,
  searchQuery
}: CardsSidebarSearchProps): ReactElement {
  const [isSearchMethodMenuOpen, setIsSearchMethodMenuOpen] = useState(false);
  const searchInputRef = useRef<HTMLInputElement | null>(null);
  const searchShellRef = useRef<HTMLDivElement | null>(null);
  const t = useT();
  const frontmatterValueCandidates = useMemo(
    () => frontmatterValueCandidatesForField(searchFrontmatterCandidates, searchFrontmatterField),
    [searchFrontmatterCandidates, searchFrontmatterField]
  );
  const searchModeOptions = useMemo(() => cardSearchModeOptions(t), [t]);
  const activeSearchModeLabel = activeCardSearchModeLabel(
    searchModeOptions,
    searchMode,
    t("cards.searchModeFullText")
  );
  const searchPlaceholder = t("cards.searchPlaceholder", { mode: activeSearchModeLabel });

  useEffect(() => {
    if (searchFocusRequest <= 0) return;
    searchInputRef.current?.focus();
  }, [searchFocusRequest]);

  useEffect(() => {
    if (!isSearchMethodMenuOpen) return;

    const close = (event: globalThis.PointerEvent): void => {
      if (searchShellRef.current?.contains(event.target as Node)) return;
      setIsSearchMethodMenuOpen(false);
    };

    window.addEventListener("pointerdown", close);

    return () => window.removeEventListener("pointerdown", close);
  }, [isSearchMethodMenuOpen]);

  return (
    <div className="cards-search" ref={searchShellRef}>
      <label className={`cards-search-input${searchError ? " cards-search-input--error" : ""}`}>
        <button
          aria-label={t("cards.searchMethod")}
          className="cards-search-mode-button"
          onClick={() => setIsSearchMethodMenuOpen((current) => !current)}
          type="button"
        >
          {activeSearchModeLabel}
        </button>
        <input
          aria-label={t("cards.search")}
          list={searchMode === "frontmatter" && frontmatterValueCandidates.length > 0 ? "cards-search-frontmatter-values" : undefined}
          onChange={(event) => onSearchQueryChange(event.target.value)}
          placeholder={searchPlaceholder}
          ref={searchInputRef}
          type="search"
          value={searchQuery}
        />
      </label>
      {isSearchMethodMenuOpen ? (
        <div className="cards-search-method-menu" role="listbox" aria-label={t("cards.searchMethod")}>
          {searchModeOptions.map((option) => (
            <button
              aria-selected={option.mode === searchMode}
              className={`cards-search-method${option.mode === searchMode ? " active" : ""}`}
              key={option.mode}
              onClick={() => {
                onSearchModeChange(option.mode);
                setIsSearchMethodMenuOpen(false);
                window.setTimeout(() => searchInputRef.current?.focus(), 0);
              }}
              role="option"
              type="button"
            >
              {option.label}
            </button>
          ))}
        </div>
      ) : null}
      {searchMode === "frontmatter" ? (
        <div className="cards-search-frontmatter">
          <select
            aria-label={t("cards.searchFrontmatterField")}
            className="search-input"
            onChange={(event) => onSearchFrontmatterFieldChange(event.target.value)}
            value={searchFrontmatterField}
          >
            <option value="">{t("cards.searchFrontmatterField")}</option>
            {searchFrontmatterFields.map((field) => (
              <option key={field} value={field}>{field}</option>
            ))}
          </select>
          {frontmatterValueCandidates.length > 0 ? (
            <datalist id="cards-search-frontmatter-values">
              {frontmatterValueCandidates.map((candidate) => (
                <option key={candidate} value={candidate} />
              ))}
            </datalist>
          ) : null}
        </div>
      ) : null}
    </div>
  );
}
